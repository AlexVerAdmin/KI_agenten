...
ADMIN_TOOLS = [
    ...
    admin_tools.execute_confirmed_command,
...
def get_model(purpose='general', model_override=None, user_id=None):
    ...
    # Поддержка Ollama через LangChain (СТРОГО ЛОКАЛЬНО ДЛЯ ПРИВАТНОСТИ)
    if model_name.startswith('ollama/'):
        if not is_ollama_online():
            error_msg = f"❌ ОШИБКА КОНФИДЕНЦИАЛЬНОСТИ: Локальный сервер Ollama ({config.local_server_url}) недоступен. Запрос прерван для защиты данных."
            print(f"DEBUG: {error_msg}")
            raise ConnectionError(error_msg)

        print(f"DEBUG: Using Local Ollama model: {model_name} at {config.local_server_url}")
        from langchain_ollama import ChatOllama
        actual_model = model_name.replace('ollama/', '')
        return ChatOllama(
            model=actual_model, 
            base_url=config.local_server_url,
            timeout=30  # Ждем не более 30 сек
        )

    if 'gemini' in model_name:
        print(f"DEBUG: Using Gemini model: {model_name}")
        from langchain_google_genai import ChatGoogleGenerativeAI
        # Gemini 3.1 Pro требует явного указания v1beta для корректной работы thought_signature
...
def node_handler(state: AgentState):
    ...
    elif agent_type == 'local_admin':
        llm_with_tools = llm.bind_tools(ADMIN_TOOLS)
    elif agent_type == 'vds_admin':
        # ДЛЯ VDS АДМИНА ОТКЛЮЧАЕМ TOOL BINDING, ЧТОБЫ ОН ГОВОРИЛ ТЕКСТОМ
        llm_with_tools = llm 
    else:
        llm_with_tools = llm
    ...
    system_prompts = {
        ...
        'vds_admin': 'Ты системный администратор сервера VDS (Debian). Твоя задача - помогать пользователю управлять ресурсами и процессами. ПРАВИЛО: Если нужно проверить ресурсы (CPU, RAM, DISK), ты ДОЛЖЕН сначала спросить у пользователя: "Я хочу выполнить [команда] для [цель]. Вы разрешаете?". Если пользователь ответит "Да", "Разрешаю" или "Ок", только тогда вызывай инструмент execute_confirmed_command напрямую в JSON формате. НИКОГДА не вызывай request_shell_execution.',
