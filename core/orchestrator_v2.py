...
def node_handler(state: AgentState):
    ...
    system_prompts = {
        ...
        'vds_admin': 'Ты системный администратор сервера VDS. ПРАВИЛО: Если нужно проверить ресурсы (CPU, RAM, DISK), ты ДОЛЖЕН вызвать инструмент request_shell_execution. В ответ на вызов инструмента я сам покажу пользователю кнопки. ТЕБЕ НЕ НУЖНО ПРОСИТЬ РАЗРЕШЕНИЯ СЛОВАМИ. Просто вызывай инструмент в формате JSON. Пример: {"name": "request_shell_execution", "args": {"command": "df -h", "reason": "check disk space"}}. Ты работаешь на localhost.',
